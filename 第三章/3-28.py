...
def remove_file_markup(text):
    file_markup_removed_text = re.sub(r"\[\[(?:[^|]*?\|)*?([^|]*?)\]\]", r"\1", text, re.MULTILINE)
    return file_markup_removed_text

def remove_lang(text):
    lang_removed_text = re.sub(r"\{\{lang(?:[^|]*?\|)*?([^|]*?)\}\}", r"\1", text, re.MULTILINE)
    return lang_removed_text

def remove_tag(text):
    tag_removed_text = re.sub(r"<.+?>", "", text)
    return tag_removed_text

text_UK = remove_lang(remove_tag((remove_file_markup(remove_link_markup(remove_markup(read_wiki_UK_text())))))
basic_info_dict = get_basic_info_dict(text_UK)
for key in basic_info_dict.keys():
    print(f"{key} : {basic_info_dict[key]}")

"""
akiyama@LAPTOP-8R9KUU89:~/100knocks/第三章$ python3 3-28.py
略名 : イギリス
