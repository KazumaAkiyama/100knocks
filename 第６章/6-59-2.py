...
nb = MultinomialNB()
nb.fit(train_feature_nparr, train_category_nparr)

#正解率計算のための関数
def accuracy(nb, feature_nparr, category_list):
    correct_count = 0
    pre_category_list = nb.predict(feature_nparr)
...
print(accuracy(nb, valid_feature_nparr, valid_category_list))
print(accuracy(nb, test_feature_nparr, test_category_list))
